...
module.exports.run = async (client, message, args) => {
  ...
  if (message.channel.id !== "410526913879080960" && message.channel.id !== "460217052339372042" && message.author.id !== "298812170093723649") return;
    ...
    try {
        ...
        console.log(err)
        message.channel.send('Hmm, that  doesn\'t seem to exist!')
        return;
                }
    let uwa = test.uuid;
    try {
        player =  ( await HypixelClient.getPlayer('name', uwa)).player
        let zzz = test.stats.SkyBlock.profiles.profile_id;
